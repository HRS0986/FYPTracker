...
export default function ResourceChart() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetched = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        // Fallback for items without explicit date if needed, though they have createdAt
  ...
  const chartData = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      if (!groups[d]) {
        groups[d] = { date: d, paper: 0, article: 0, video: 0, "ai-chat": 0, document: 0 };
      }
    ...
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all h-[400px] flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-900 dark:text-slate-100">Resource Activity</h3>
        </div>
            ...
            <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
              {["week", "month", "all"].map((r) => (
                <button
                  ...
                  className={`w-full px-4 py-2 text-left text-sm font-medium transition-colors ${range === r ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
                >
